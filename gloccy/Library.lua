...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(11, 27, 42),
        MainColor = Color3.fromRGB(22, 42, 60),
        AccentColor = Color3.fromRGB(35, 110, 220),
        OutlineColor = Color3.fromRGB(38, 72, 102),
        FontColor = Color3.fromRGB(232, 243, 255),
...
for _, IconSource in ipairs({
    "https://raw.githubusercontent.com/devsonic46-create/lib/main/gloccy/lucide-icons.lua",
    "https://gitlab.com/upio/lucide-roblox-direct/-/raw/main/source.lua",
}) do
    local Success, Module = pcall(function()
        return (loadstring(game:HttpGet(IconSource)) :: () -> IconModule)()
    end)
...
--// Cursor
local Cursor, CursorCustomImage, CursorPosition
do
    Cursor = New("Frame", {
    ...
    --// Cursor position tracking (mouse + touch) \\--
    CursorPosition = Vector2.new(Mouse.X, Mouse.Y)
    local UsingTouchInput = false
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        do
            ...
            New("UIGradient", {
                Color = ColorSequence.new({
                    ...
                    ColorSequenceKeypoint.new(0.3, Color3.fromRGB(50, 100, 255)),
                    ColorSequenceKeypoint.new(0.6, Color3.fromRGB(120, 40, 255)),
        ...
        if WindowInfo.Center then
            WindowGroup.Position = UDim2.new(0.5, -WindowGroup.Size.X.Offset / 2, 0.5, -WindowGroup.Size.Y.Offset / 2)
        end
        ...
        Library:MakeDraggable(WindowGroup, TopBar, false, true)
        ...
        local function CreateWindowControl(Icon, HoverColor, HoverIconColor)
            local Button = New("TextButton", {
                AutoButtonColor = false,
                BackgroundColor3 = "MainColor",
                BackgroundTransparency = 1,
            ...
            Button.MouseEnter:Connect(function()
                ...
                TweenService:Create(Stroke, Library.TweenInfo, { Transparency = 0.3 }):Play()
                TweenService:Create(IconLabel, Library.TweenInfo, {
                    ImageColor3 = HoverIconColor,
                    ImageTransparency = 0,
                }):Play()
            end)
            Button.MouseLeave:Connect(function()
                ...
                TweenService:Create(Stroke, Library.TweenInfo, { Transparency = 0.7 }):Play()
                TweenService:Create(IconLabel, Library.TweenInfo, {
        ...
        if MinimizeIcon then
            local MinimizeButton = CreateWindowControl(MinimizeIcon, Library.Scheme.AccentColor, Library.Scheme.AccentColor)
        ...
        if CloseIcon then
            local CloseButton = CreateWindowControl(CloseIcon, Color3.fromRGB(255, 82, 82), Color3.fromRGB(255, 128, 128))
        ...
        if WindowInfo.Resizable then
            ...
            Library:MakeResizable(WindowGroup, ResizeButton, function()
                for _, Tab in Library.Tabs do
                    Tab:Resize(true)
                end
            end)
    ...
    --// Window Visibility (Fade In/Out) \\--
    local FadeTweenInfo = TweenInfo.new(0.13, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local FadeScale = New("UIScale", {
        Parent = WindowGroup,
    })
    ...
    local function SetWindowVisible(Visible)
        if Visible then
            ...
            TweenService:Create(WindowGroup, FadeTweenInfo, {
                GroupTransparency = 0,
            }):Play()
            TweenService:Create(FadeScale, FadeTweenInfo, {
                Scale = 1,
            }):Play()
        else
            TweenService:Create(WindowGroup, FadeTweenInfo, {
    ...
    function Window:SetCornerRadius(Radius: number)
        ...
        for _, UICorner in Library.Corners do
            if UICorner:GetAttribute("GroupboxCorner") then
                UICorner.CornerRadius = UDim.new(0, math.max(Radius, 12))
            elseif UICorner.CornerRadius.Offset == Library.CornerRadius / 2 then
    ...
    function Window:SetSidebarWidth(Width)
        Width = math.clamp(Width, 48, WindowGroup.Size.X.Offset - WindowInfo.MinContainerWidth - 1)
    ...
    function Window:ShowTabInfo(Name, Description)
        ...
        if IsDefaultSearchbarSize and SearchBox then
    ...
    function Window:AddTab(...)
        ...
        local IconIsCustom = Icon and Icon.Custom or false
        do
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                local GroupboxCornerRadius = math.max(WindowInfo.CornerRadius, 12)
                ...
                local GroupboxCorner = New("UICorner", {
                    CornerRadius = UDim.new(0, GroupboxCornerRadius),
                    Parent = GroupboxHolder,
                })
                GroupboxCorner:SetAttribute("GroupboxCorner", true)
                table.insert(Library.Corners, GroupboxCorner)
        ...
        function Tab:AddTabbox(Info)
            ...
            do
                local TabboxCornerRadius = math.max(WindowInfo.CornerRadius, 12)
                ...
                local TabboxCorner = New("UICorner", {
                    CornerRadius = UDim.new(0, TabboxCornerRadius),
                    Parent = TabboxHolder,
                })
                TabboxCorner:SetAttribute("GroupboxCorner", true)
                table.insert(Library.Corners, TabboxCorner)
        ...
        function Tab:Hide()
            ...
            if TabIcon then
                TweenService:Create(TabIcon, Library.TweenInfo, {
                    ImageColor3 = IconIsCustom and Library.Scheme.WhiteColor or Library.Scheme.AccentColor,
                    ImageTransparency = 0.5,
                }):Play()
            end
            if TabAccent then
    ...
    function Window:AddKeyTab(...)
        ...
        function Tab:Show()
            ...
            TweenService:Create(TabGroup, Library.TweenInfo, {
                GroupTransparency = 0,
            }):Play()
    ...
    function Window:Toggle(Value: boolean?)
        ...
        SetWindowVisible(Library.Toggled)
